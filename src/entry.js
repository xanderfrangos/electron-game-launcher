...
const appStart = (event, args) => {
    ...
    let db = window.gamesDB.steam;
    let dbArr = Object.values(window.gamesDB.steam)
    ...
    const fullscreenItem = new UIItem(() => {window.isFS = (window.isFS ? false : true); window.fsEventRecieved(false, window.isFS)});
...
setTimeout(() => { window.appStart()}, 333)